...
msdak8sConfigProcessor.prototype.onStart = function (success) {
    ...
    try {
        fs.access(msdak8sOnPollingSignal, fs.constants.F_OK, function (err) {
            if (err) {
                logger.fine("MSDAk8s: OnStart, the polling signal is off. ", err.message);
            } else {
                logger.fine("MSDA k8s audit onStart: ConfigProcessor started, clear the signal.");
                fs.unlinkSync(msdak8sOnPollingSignal);
            }
        });
    } catch(err) {
...
msdak8sConfigProcessor.prototype.onPost = function (restOperation) {
    ...
    if (global.msdak8sOnPolling.includes(inputPoolName)) {
        ...
        logger.fine("MSDA onPost: set msdak8sOnpolling signal: ", global.msdak8sOnPolling);
    }
                    ...
                    if (nodeAddress.length !== 0) {
                        ...
                        mytmsh.executeCommand("tmsh -a list ltm pool " + inputPoolName)
                            .then(function () {
                                ...
                                // Error handling - Set the block as 'ERROR'
                            .catch(function (error) {
                                logger.fine("MSDA: onPost, clear pool failed: " + error.message);
                            });
                    }
                }, function (err) {
